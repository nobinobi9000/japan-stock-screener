...
10. **✅ 日曜日にも休場通知が届いてしまう — 修正済み（2026-09-06、commit `6737c4f`）**

    `scripts/check_market_day.py`の`main()`が、土曜（`weekday()==5`）専用の分岐で
    `mode="cache_warm"`に切り替えて通知対象外にしていた一方、日曜（`weekday()==6`）には
    対応する分岐が無く、デフォルトの`else`（`mode="screen"`）に落ちていた。
    `is_market_open()`が「日曜日だから休場」と正しく判定した結果、
    `mode == "screen" and not is_open`の通知条件を満たしてDiscordへ休場通知が
    送信されてしまっていた。休場通知は本来「平日なのに祝日等で予定外に休場」を
    知らせるためのもので、土日の休場は自明なため通知不要という設計意図に対し、
    土曜と日曜で扱いが非対称になっていたのが原因（ユーザー指摘により発覚）。

    なお自動実行経路（cloudflare-watchdogは平日のみ、GitHub純正cronは土曜のみ）では
    日曜にこのスクリプトが動く経路が無いため、実際に観測された日曜通知はおそらく
    手動での`workflow_dispatch`実行時に発生したものと考えられる。

    **修正内容**: 日曜用に`mode="none"`の分岐を追加し、土曜と同様に通知対象
    （`screen`モード）から除外した。`mode="none"`は`shard-screen`・
    `shard-cache-warm`いずれの実行条件にも合致しないため、日曜に手動実行された
    場合もスキャン・キャッシュ更新のどちらも走らない（何もしないのが正しい挙動）。

    **検証**: 土曜・日曜・平日通常・平日祝日（元日）の4パターンをユニットテストで
    確認。平日祝日は従来通り`mode="screen"`・通知ありのまま維持されている。
...
- **有償プランのDiscord配信廃止・プラン定義の統一（2026-09-25）**

  有償の主軸を「Web分析ページ（認証）」とし、有償向けDiscordチャンネル(#full-report/#analysis/
  #premium/#chart-analysis)への配信を停止（`daily_screen.yml`の該当Webhook環境変数を削除。コードは
  Webhook未設定を自動スキップ）。Discordは無料#daily-picksのみ。Stripeはベーシック(¥980)のみ、
  プレミアムは「準備中」で全所（LP・旧LP・特商法）を統一し、「BT参考統計グラフ」を削除（原則1）。
  利用規約の登録・解約条項に「有料プランはメール登録・Stripeプラン管理画面から解約」を追記、
  重要告知は有料会員にメールでも通知する旨をプライバシーポリシーに追記（原則4）。

  **INTEGRATION_MAP.mdへの反映内容**:
  1. プラン定義の正: Free=厳選3銘柄+サマリー+Discord #daily-picks／Basic(¥980・Stripe)=Web全銘柄分析／
     Premium=準備中（Stripe商品・webhookのplanマッピング未実装）
